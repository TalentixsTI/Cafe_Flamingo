...
class CompanyElectronic(models.Model):
    ...
    def write(self, vals):
        if vals.get('date_expiration_sign') or vals.get('range_days'):
            cron = self.env.ref('factura_electronica_cr.ir_cron_send_expiration_notice', False)
    ...
    def _cron_send_email_notifications(self):
        ...
        if today >= range_date:
            template = self.env.ref('factura_electronica_cr.email_template_edi_expiration_notice', False)
    ...
    def action_get_economic_activities(self):
        _logger.debug('FECR - VAT- url: %s', self.vat)
