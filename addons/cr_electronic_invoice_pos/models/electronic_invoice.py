...
from odoo.addons.factura_electronica_cr.models import api_facturae
...
class PosConfig(models.Model):
    ...
    def create_sequences(self):
        _logger.info('Entrando a create_sequences en PosConfig')
        if self.journal_id:
            ...
            if inv_cedula:
                ...
                tipo_doc = '01'
                _logger.debug('Creando secuencia de factura electrónica')
                ...
                tipo_doc = '03'
                _logger.debug('Creando secuencia de nota de crédito electrónica')
                ...
                tipo_doc = '04'
                _logger.debug('Creando secuencia de tiquete electrónico')
...
class PosOrder(models.Model):
    ...
    def action_invoice_sent(self):
        _logger.info('Entrando a action_invoice_sent en PosOrder')
    ...
    def sequence_number_sync(self, vals):
        _logger.info('Entrando a sequence_number_sync en PosOrder')
    ...
    def _order_fields(self, ui_order):
        _logger.info('Entrando a _order_fields en PosOrder')
        ...
        if ui_order.get('statement_ids'):
            payment_data = ui_order['statement_ids'][0][2]
            vals['payment_method_id'] = payment_data.get('payment_method_id')
    ...
    def create(self, vals):
        _logger.info('Inicio de create en PosOrder')
        _logger.info('Valores recibidos: %s', vals)

        number_electronic = vals.get('number_electronic', False)
        if vals.get('pos_order_id', False):
            _logger.info('La orden tiene un pos_order_id: %s', vals['pos_order_id'])
            vals['number_electronic'] = '/'
        elif number_electronic:
            _logger.info('La orden tiene un número electrónico: %s', number_electronic)
            self.sequence_number_sync(vals)
            if self.env['pos.order'].search([('number_electronic', 'like', number_electronic[21:41])]):
                _logger.info('Número electrónico %s ya existe, generando uno nuevo', number_electronic)
                vals['number_electronic'] = self.env['ir.sequence'].next_by_code('pos.order.recovery')
        
        order = super(PosOrder, self).create(vals)
        _logger.info('Orden creada con ID: %s', order.id)
        _logger.info('Fin de create en PosOrder')
        return order

    def _process_order(self, order, existing_order):
        _logger.info('Procesando orden POS')

        # Lógica para determinar tipo de documento
        if not order.get('partner_id') or not order['partner_id']:
            order['tipo_documento'] = 'TE'  # Tiquete Electrónico

            # Procesar información de pago
        if order.get('statement_ids'):
            payment_data = order['statement_ids'][0][2]
            order['payment_method_id'] = payment_data.get('payment_method_id')

        # Llamar al método original y obtener el ID de la orden creada
        order_id = super(PosOrder, self)._process_order(order, existing_order)

        # Obtener el objeto de la orden
        pos_order = self.browse(order_id)

        # Asignar número electrónico según el tipo de documento
        if pos_order.tipo_documento == 'FE':
            pos_order.number_electronic = pos_order.session_id.config_id.FE_sequence_id.next_by_id()
        elif pos_order.tipo_documento == 'TE':
            pos_order.number_electronic = pos_order.session_id.config_id.TE_sequence_id.next_by_id()
        else:
            pos_order.number_electronic = pos_order.session_id.config_id.NC_sequence_id.next_by_id()

        pos_order.sequence = pos_order.number_electronic[21:41]

        return order_id

    def action_pos_order_paid(self):
        _logger.info('Inicio de action_pos_order_paid')
        for order in self:
            _logger.info('Procesando orden: %s', order.id)
            if not order.pos_order_id:
                _logger.info('La orden %s no tiene pos_order_id, continuando con la siguiente', order.id)
                continue
            
            if order.tipo_documento == 'FE':
                order.number_electronic = order.session_id.config_id.FE_sequence_id.next_by_id()
                _logger.info('Asignado número electrónico FE: %s a la orden %s', order.number_electronic, order.id)
            elif order.tipo_documento == 'TE':
                order.number_electronic = order.session_id.config_id.TE_sequence_id.next_by_id()
                _logger.info('Asignado número electrónico TE: %s a la orden %s', order.number_electronic, order.id)
            else:
                order.tipo_documento = 'NC'
                order.number_electronic = order.session_id.config_id.NC_sequence_id.next_by_id()
                _logger.info('Asignado número electrónico NC: %s a la orden %s', order.number_electronic, order.id)
            
            order.sequence = order.number_electronic[21:41]
            _logger.info('Asignado número de secuencia: %s a la orden %s', order.sequence, order.id)

        result = super().action_pos_order_paid()
        _logger.info('Fin de action_pos_order_paid')
        return result
