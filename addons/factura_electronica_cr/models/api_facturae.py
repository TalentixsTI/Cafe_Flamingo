...
def get_token_hacienda(inv, tipo_ambiente):
    ...
    token_hacienda = None  # Inicializar la variable
...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
    ...
    _logger.info('Api_facture 2')
    codigo_actividad = (
    inv.economic_activity_id and
    re.match(r'^\d{6}$', str(inv.economic_activity_id.code)) and
    inv.economic_activity_id.code or '000000'
    )
    ...
    #sb.append('<CodigoActividad>' + str(codigo_actividad) + '</CodigoActividad>')
    sb.append('<CodigoActividad>552004</CodigoActividad>')

    sb.append('<NumeroConsecutivo>' + str(inv.number_electronic[21:41]) + '</NumeroConsecutivo>')
    sb.append('<FechaEmision>' + str(inv.date_issuance) + '</FechaEmision>')
    sb.append('<Emisor>')
    ...
    sb.append('<Numero>' + str(issuing_company.vat or '') + '</Numero>')
    sb.append('</Identificacion>')
    sb.append('<NombreComercial>' + escape(str(issuing_company.commercial_name or 'NA')) + '</NombreComercial>')
    sb.append('<Ubicacion>')
    sb.append('<Provincia>' + str(issuing_company.state_id.code or '') + '</Provincia>')
    sb.append('<Canton>' + str(issuing_company.county_id.code or '') + '</Canton>')
    sb.append('<Distrito>' + str(issuing_company.district_id.code or '') + '</Distrito>')
    ...
    sb.append('<CorreoElectronico>' + str(issuing_company.email or 'NA') + '</CorreoElectronico>')
    sb.append('</Emisor>')

    if inv.tipo_documento == 'TE' or (inv.tipo_documento == 'NC' and not receiver_company.vat):
        pass
    else:
        vat = re.sub('[^0-9]', '', str(receiver_company.vat or ''))
        if not receiver_company.identification_id:
            ...
            id_code = str(receiver_company.identification_id.code)
    ...
    for payment_method in payment_methods_id:
        sb.append('<MedioPago>' + str(payment_method) + '</MedioPago>')
    ...
    if lines:
        ...
        for (k, v) in lines.items():
            ...
            if v.get('codigoCabys'):
                sb.append('<Codigo>' + str(v['codigoCabys']) + '</Codigo>')
            ...
            if v.get('impuesto'):
                for (a, b) in v['impuesto'].items():
                    ...
                    if inv.tipo_documento != 'FEE' and b.get('exoneracion'):
                        ...
                        sb.append('<TipoDocumento>' + str(receiver_company.type_exoneration.code) + '</TipoDocumento>')
                        sb.append('<NumeroDocumento>' + str(receiver_company.exoneration_number) + '</NumeroDocumento>')
                        sb.append('<NombreInstitucion>' + str(receiver_company.institution_name) + '</NombreInstitucion>')
...
def consulta_documentos(self, inv, env, token_m_h, date_cr, xml_firmado):
    ...
    if inv.tipo_documento != 'FEC' and estado_m_h == 'aceptado' and (not last_state or last_state == 'procesando'):
        ...
        if inv.move_type in ['in_invoice', 'in_refund']:
            email_template = self.env.ref('factura_electronica_cr.email_template_invoice_vendor', False)
        else:
