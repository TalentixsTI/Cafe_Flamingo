...
class PosOrders(models.TransientModel):
    ...
    validate_pin = fields.Char(string='Enter Code')

    set_paid_delete_order = fields.Selection([
        ('without_code', 'Delete POS Order Without Code'),
        ('with_code', 'Delete POS order with Code')], string="POS Pricelists", related="company_id.set_paid_delete_order", readonly=False)
    ...
    def call_delete_order(self, record):
            ...
            for payment in record.payment_ids:
                ...
                for journal_entry in draft_moves:
                    ...
                    for line in MoveLine:
                        if payment.payment_method_id.journal_id.id == line.journal_id.id:
        ...
        if record.picking_ids:
            picking = self.env['stock.picking'].search([('name', 'in', record.picking_ids.mapped('name'))])
            for pick in picking.move_ids_without_package:
                pick.write({'state': 'draft', 'quantity': 0})
